...
import { MOCK_ENCUESTAS } from '../data/constants';
import NuevaEncuestaModal from './modals/NuevaEncuestaModal';
...
const COLUMNS = [
  ...
  { key: 'fechaFin',    title: 'Fecha Fin',    sortable: true  },
  { key: 'actions',     title: '',             sortable: false },
];

// Convierte fechaISO + horaHHMM al formato de display "YYYY/MM/DD - HH:MM"
function formatDateTime(dateISO, timeHHMM) {
  if (!dateISO) return '—';
  const [y, m, d] = dateISO.split('-');
  return `${y}/${m}/${d} - ${timeHHMM || '00:00'}`;
}
...
export default function EvaluationList() {
  const { abrirPasoGeneral } = useEvaluation();
  ...
  const filtered = encuestas.filter(enc => {
    ...
    return enc.titulo.toLowerCase().includes(q);
  });

  function handleNuevaEncuesta(data) {
    const nueva = {
      id: Math.max(0, ...encuestas.map(e => e.id)) + 1,
      titulo: data.nombre,
      fechaInicio: formatDateTime(data.fechaInicio, data.horaInicio),
      fechaFin:    formatDateTime(data.fechaTermino, data.horaTermino),
    };
    setEncuestas(prev => [nueva, ...prev]);
    abrirPasoGeneral(data);
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ overflowX: 'auto' }}>
            <table
              aria-label="Listado de encuestas"
              style={{
                width: '100%',
      ...
      {/* Modal Nueva encuesta — se superpone sobre esta vista */}
      <NuevaEncuestaModal
        isOpen={showModal}
        onClose={() => setShowModal(false)}
        onSubmit={handleNuevaEncuesta}
      />
    </div>
  );
}
...
// ── EncuestaRow ───────────────────────────────────────────────────────────────
function EncuestaRow({ enc, tdBase }) {
  ...
  return (
    ...
    >
      {/* ID */}
      <td style={{ ...tdBase, paddingLeft: p5, color: C.grisOscuro, fontSize: T.sz13, width: '72px' }}>
        {enc.id}
      </td>
      ...
      {/* Fecha Inicio */}
      <td style={{ ...tdBase, color: C.grisOscuro, minWidth: '160px' }}>
        {enc.fechaInicio}
      </td>
