...
export function EvaluationProvider({ children }) {
  const [view, setView] = useState('list'); // 'list' | 'wizard' | 'general' | 'notificaciones' | 'crearNotificacion' | 'resumen'
  ...
  const [tipoNotificacionActual, setTipoNotificacionActual] = useState(null);
  const [notificacionEditar, setNotificacionEditar] = useState(null);
  const [surveyCompletedSteps, setSurveyCompletedSteps] = useState([]);

  const markSurveyStepComplete = useCallback((key) => {
    setSurveyCompletedSteps(prev => prev.includes(key) ? prev : [...prev, key]);
  }, []);
  ...
  const abrirPasoGeneral = useCallback((modalData) => {
    setEncuestaActual(modalData);
    setSurveyCompletedSteps([]);
    setView('general');
  }, []);
  ...
  const abrirCrearNotificacion = useCallback((tipo) => {
    setTipoNotificacionActual(tipo);
    setNotificacionEditar(null);
    setView('crearNotificacion');
  }, []);

  const editarNotificacion = useCallback((notif) => {
    setNotificacionEditar(notif);
    setTipoNotificacionActual(notif.tipo);
    setView('crearNotificacion');
  }, []);

  const guardarNotificacion = useCallback((data) => {
    ...
    setNotificacionEditar(null);
    setTipoNotificacionActual(null);
    setView('notificaciones');
  }, []);
  ...
  return (
    <EvaluationContext.Provider value={{
      ...
      encuestaActual, setEncuestaActual,
      abrirPasoGeneral,
